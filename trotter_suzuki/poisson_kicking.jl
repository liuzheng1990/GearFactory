...
using JLD
import Plots: heatmap, plot, plot!, title!, savefig

vg = Vec_Hs[:,1];
Cm0 = svector2statematrix(0,vg,m_min,m_max)
t0 = 0.0; tf = 150.0; dt = 0.01;
...
function poissonkicking(Cm0::StateMatrix, k_step::Int, rate::Real, t0::Real, tf::Real, dt::Real, nsample::Int,
	...
	dt_step_list = Float64[]
	t = t0
	dt_step = poissontimestep(rate)
	push!(dt_step_list,dt_step)
	Cm_current = Cm0
	...
	return t_list, mean_L1_list, mean_L2_list, variance_L1_list, variance_L2_list, t_sample_list, statesample_list, dt_step_list
end #function

nsample = 10
for i=1:20
	println("$(i) out of 20...")
	t_list,mean_L1_list, mean_L2_list, variance_L1_list, variance_L2_list, t_sample_list, statesample_list, dt_step_list=
			poissonkicking(Cm0, k_step, rate, t0, tf, dt, nsample, m_min, m_max)

			# length(dt_step_list)
	plot(t_list,mean_L1_list,label="mean L1")
	plot!(t_list,mean_L2_list,label="mean L2")
	title!("k_step=$(k_step), rate=$(rate); #kickings = $(length(dt_step_list))")
	savefig("Trial $(i):k_step=$(k_step), rate=$(rate).png")
end

Cm_temp = circshift(Cm0,[40,0])
heatmap(abs2.(Cm_temp))
